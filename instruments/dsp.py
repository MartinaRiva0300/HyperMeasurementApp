...
`FWHM_apodization.m` (instrument-lineshape FWHM = FFT of the window).

The windows are a verbatim port of `Apodization.m`: a window of full period
`size` centred on the ZPD sample `center` (x = index - center), so an off-centre
burst gives a window centred on `center` -- Happ-Genzel additionally zeroed
beyond +/- size/2, Triangular zeroed past its base. `center` is computed by the
caller (the ZPD / centre-burst finder), so no width parameter is needed.
"""
from __future__ import annotations

import numpy as np

# Apodization window types (Apodization.m cases 1-4). No width parameter -- the
# window is fixed by its family and the ZPD centre. 'boxcar' = no apodization.
APOD_TYPES = [
    "happ-genzel",
    "blackman-harris-3",
    "blackman-harris-4",
...
def apodization_window(apod_type, size, center):
    """Apodization window of length `size`, centred on the ZPD sample `center`.

    Verbatim port of Apodization.m (cases 1-4). `x = index - center`; the cosine
    windows have period `size`. Happ-Genzel is zeroed beyond +/- size/2.
    `boxcar`/unknown -> ones (no apodization).
    """
    ...
    if apod_type == "triangular":
        # MATLAB: x(end)-|x| if the centre sits past the midpoint, else center-|x|.
    ...
    # boxcar / none / unknown -> no apodization.
    return np.ones(size)


def apodization_window_map(apod_type, size, center):
    """Per-pixel apodization: `center` is an (h, w) ZPD-index map, returns an
    (size, h, w) window with each pixel centred on its OWN ZPD. Same MATLAB
    window families as apodization_window, vectorised across the field.
    """
    apod_type = str(apod_type).lower()
    size = int(size)
    ...
    if apod_type == "blackman-harris-4":
        return (0.35875 + 0.48829 * np.cos(2.0 * np.pi * x / size)
                ...
                + 0.01168 * np.cos(3.0 * 2.0 * np.pi * x / size))
    if apod_type == "triangular":
        peak = np.where(center > np.round(size / 2.0), x[-1], center)  # (h, w)
